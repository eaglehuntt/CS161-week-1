...
def estimate_population(years):
    ...
    """
    # Calculate the total number of seconds in the given number of years
    ...
    total_births = total_seconds * BIRTH_RATE
    total_deaths = total_seconds * DEATH_RATE
    total_immigrants = total_seconds * IMMIGRANT_RATE

    # Add the birth and immigration rates and subtracting the death rate
    estimated_population = CURRENT_US_POPULATION + total_births + total_immigrants - total_deaths
...
print(f"Estimated population after {inputted_years} years: {estimate_population(inputted_years):,}")
